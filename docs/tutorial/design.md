...
- **对齐纹理**：墙面的纹理没有对齐，“纹理调整工具”也提供了辅助对齐的方法。
	- 调整对齐主要是调整缩放、移动和旋转的值，但一般不需要手动填写，hammer给了一些辅助的按钮，你也可以通过点选输入框之后，按```↑```和```↓```键来调整。
		- ![](../images/hammer_tool_texture_align_param.png)
		- 取值范围包含负数。对于缩放部分，负数值会使纹理发生镜像变化，以下图中DANGER纹理为例，上半部分的X轴缩放比例为0.54，纹理没有镜像处理，下半部分的X轴缩放比例为-0.54，该缩放比例的值为负数，纹理发生了镜像处理。
		- ![](../images/scale_reverse.png)
...
?> **组**：如果你发现选择盒子时，六个墙壁都同时选中了，这是因为他们属于同一个“组”。同一个”组“的物体会自动同时选中，方便你同时操作。你可以在快捷功能栏打开”忽略组“![](../images/hammer_shortcut_ignore_group.png)按钮来暂时禁止同时选中组，也可以在选中后点击”解散当前组“![](../images/hammer_shortcut_ungroup.png)，或者按下```Ctrl+U```来解散当前组。相反地，点击“将选择的对象组成组”![](../images/hammer_shortcut_group.png)，或者按下```Ctrl+G```则会将当前选中的物体组建成一个组。
